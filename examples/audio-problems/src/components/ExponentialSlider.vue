<template>
    ...
    <div class="slider-container" @pointerenter="$emit('pointerenter', label)" @pointerleave="$emit('pointerleave')">
        <div class="d-flex justify-content-between w-100">
            <label :for="label" id="slider-label" class="slider-text">
                {{ formattedLabel }}
            </label>
            <span id="slider-value" class="slider-text" v-show="isClicked">{{ sliderValue }}</span>
        </div>
        <input type="range" class="form-range" :id="label" :min="realStep" max="1" :value="realValue" :step="realStep"
...
export default {
    ...
    created () {
        this.validValues.push(Number(this.min));
        let maxReached = false;
        let item = 0;
        while (!maxReached) {
            let nextExpItem = Math.pow(this.expStep, item);
            if (nextExpItem > this.min) {
                this.validValues.push(nextExpItem);
            }
            item++;
            if (nextExpItem >= this.max) {
                maxReached = true;
                if (this.validValues.slice(-1) != this.max) {
                    this.validValues.push(Number(this.max));
                }
            }
        }

        this.realStep = 1 / this.validValues.length;
        this.validValuesNorm = [...Array(this.validValues.length).keys()].map( v => (v+1) / this.validValues.length);
        this.realValue = this.validValuesNorm[this.validValues.indexOf(Number(this.initialValue))]
    }
}
</script>
