...
EMSCRIPTEN_BINDINGS(Class_Extractor) { 
  class_<PolarFFT>("PolarFFT")
    .constructor<int>()
    .property("version", &PolarFFT::essentiaVersion)
    .function("configure", &PolarFFT::configure)
    .function("compute", &PolarFFT::compute)
    .function("reset", &PolarFFT::reset)
    .function("shutdown", &PolarFFT::shutdown)
    ;
